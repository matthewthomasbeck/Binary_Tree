 ...
 */
void rebalanceTree(Tree* t, TNode* x) {                                                                                          //DONE
  
  while (x != NULL) {
  
    int balanceX = getBalance(x);
 ...
 */
TNode* getTallerSubTree(TNode* root) {                                                                                          //DONE
    ...
    if (root->pRight->height >= root->pLeft->height) {
...
bool isSameSignBalance(TNode* x, TNode* z) {                                                                                          //DONE
 ...
 *
 */
void printHuffmanEncoding( TNode* root, char c ) {                                                                                          //DONE
  
  if (root->pLeft == NULL || root->pRight == NULL) { // if there are no child nodes to compare 'c' with 'str[0]'
    
    return;
  }
  
  else { // if there is data to compare
  
    int i;
  
    for (i = 0; i < strlen(root->pLeft->str); i++) { // loop len of left node
  
      if (c == root->pLeft->str[i]) { // if left node has correct 'c' in 'str[0]'
      
        printf("0");
        
        printHuffmanEncoding(root->pLeft, c); // continue down left path
      }
    }
    
    for (i = 0; i < strlen(root->pRight->str); i++) { // loop len of right node
    
      if (c == root->pRight->str[i]) { // if right node has correct 'c' in 'str[0]'
      
        printf("1");
        
        printHuffmanEncoding(root->pRight, c); // continue down right path
      }
    }
  }
}
 ...
 */
void insertSegment( TNode* root, double segmentStart, double segmentEnd ){                                                                                          //DONE
  ...
  else if ((segmentStart < root->low && segmentEnd < root->low) || (segmentStart > root->high && segmentEnd > root->high)) { // else if the given segment is completely to the left or right of the current node's range, return
  
    return;
  }

  else if (segmentStart <= root->low && segmentEnd >= root->high) { // else if the given segment completely covers the range represented by this node: increase cnt by 1 and return
 ...
 */
int lineStabQuery( TNode* root, double queryPoint ){                                                                                          //DONE
  ...
  else if (queryPoint < root->low || queryPoint > root->high) { // else if the queryPoint is completely to the left or right of the current node's range: return 0.
