...
#define PRINT_AVL_TREE true     /* set to true to enable printing on AVL trees after inserting all of the data */
...
int carTraversalTree( double moveSequence[], int numMoves ){
    ...
    for( i=0 ; i<numMoves; i++){
        insertSegment( root, segmentStartArray[i], segmentEndArray[i] ); //TO DO FUNCTION CALLED HERE
    ...
    {
        int temp = lineStabQuery( root, points[i] ); //TO DO FUNCTION CALLED HERE
        if( temp>max ){
            max = temp;
        }
    }
